...
int main() {
    ...
    // Full test: allocate, free, and perform RDMA operations using multiple compute servers
    ...
    // --- Baseline KV-index RDMA Test ---
    std::cout << "[KVIndexRDMA] Baseline KV-index RDMA test..." << std::endl;
    KVIndexRDMA kv_index(rdma_mgr, 0); // Use server 0 for demonstration
    ...
    // Update
    for (const auto& kv : kv_pairs) {
        std::string new_val = kv.second + "_updated";
