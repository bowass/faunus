...
bool ShermanIndex::try_lock_address(GlobalAddress lock_address) {
    ...
    // Acquire RDMA lock (only if not handed over)
...
void ShermanIndex::write_page_and_unlock(void* page_buffer, GlobalAddress page_address, size_t page_size, GlobalAddress lock_addr) {
    ...
    bool should_unlock_remote = true;
    if (local_lock_mgr_) {
        bool handover = local_lock_mgr_->can_hand_over(lock_addr);
        if (handover) {
            should_unlock_remote = false;
        }
    }

    if (should_unlock_remote) {
    ...
    if (local_lock_mgr_) {
        local_lock_mgr_->release(lock_addr);
    }
}
...
void ShermanIndex::print_tree(size_t offset, int depth, bool show_kv) {
    ...
    LeafPage page;
    assert(rdma_read_object(*rdma_mgr_, offset, page));
    if (page.hdr.leftmost_ptr == GlobalAddress::Null()) {
        std::cout << indent << "Leaf Page at " << std::hex << offset << std::dec 
                  << " [level=" << page.hdr.level << ", keys=" << (page.hdr.last_index + 1) << "]\n";
        for (int i = 0; i <= page.hdr.last_index; ++i) {
            auto &r = page.records[i];
            if (!r.is_empty()) {
                KVItem item;
                assert(rdma_read_object(*rdma_mgr_, r.kv_ptr, item));
                std::cout << indent << "  Key: " << item.key << " -> Value: " << item.value << "\n";
            }
        }
        if (page.hdr.sibling_ptr != GlobalAddress::Null()) {
            print_tree(page.hdr.sibling_ptr, depth, show_kv);
        }
    } else {
        std::cout << indent << "Internal Page at " << std::hex << offset << std::dec 
                  << " [level=" << page.hdr.level << ", keys=" << (page.hdr.last_index + 1) << "]\n";
        auto internal_page = reinterpret_cast<InternalPage*>(&page);
        print_tree(internal_page->hdr.leftmost_ptr, depth + 1, show_kv);
        for (int i = 0; i <= internal_page->hdr.last_index; ++i) {
            std::cout << indent << "  Key: " << internal_page->records[i].key << "\n";
            print_tree(internal_page->records[i].ptr, depth + 1, show_kv);
        }
        if (internal_page->hdr.sibling_ptr != GlobalAddress::Null()) {
            print_tree(internal_page->hdr.sibling_ptr, depth, show_kv);
        }
    }
}
